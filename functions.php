...
function water_press_scripts() {
    ...
    $version = $theme_name['Version'];

    $font_query_args = array(
		'family' => 'Roboto+Condensed:400,700|Roboto:400,700,300,900',
	);

    wp_enqueue_style( 'water-press-google-fonts', add_query_arg( $font_query_args, "//fonts.googleapis.com/css" ) );
    wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/fonts/genericons/genericons/genericons.css' );
    wp_enqueue_style( 'water-press-style', get_stylesheet_uri(), '', $version );

    wp_enqueue_script( 'masonry' );
    wp_enqueue_script( 'jquery-one-page-nav', get_template_directory_uri() . '/js/jquery.nav.js', array('jquery'), '3.0.0', true );
